<template>
    ...
    width: auto !important; box-shadow: inset 0 0 0 1000px rgba(0, 79, 253, 0.4); ;
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <div class="container-fluid">
                            <div class="row clearmargin clearpadding row-image-txt" style="height:178px; width: auto;"
                                 v-if="now_playing">
                                <div v-if="now_playing"
                                     class="col-xs-12 col-sm-6 col-md-6 clearmargin clearpadding col-sm-push-6"
                                     :style="'background: url('+now_playing.image+');background-size:100%;height:242px;background-position:center;width:50%;'">
                                    <div></div>
                                    <p style="margin-top:.5EM;margin-left:.3em;font-family:'Circular-medium';width:100%;text-shadow:-1px -1px 25px rgb(1,1,1);padding:1em; color: white;">
                                        <i class="ion-location"></i>&nbsp Live from {{current_state}}</p>
                                    <div style="cursor:pointer;display:-ms-inline-flexbox;display:inline-flex;-ms-flex-align:center;align-items:center;-ms-flex-pack:center;height:3em;width:9em;border-radius:40px;font-size:1.09em;color:#fff;margin-top:7em;margin-left:1em;text-align:center;background-color:red;"><span
        ...
        margin-right: 0.4em;
        width: 27px;
        border-radius: 50%;"><i :class="'ion-ios-'+play+''"
                                @click="playthis(0,'livestream', 'http://icestream.coolwazobiainfo.com:8000/coolfm-lagos', now_playing.title);"
                                style="margin-left: -1em;"></i></span>
                                        <p style="margin-top:.8EM;margin-left:.3em;font-family:'Circular-medium';padding-top: 0.2em;">
                                            Listen Live</p>
    ...
    animation: panning 50s infinite linear;
            '>
                                    <h1 v-if="now_playing"
                                        style="font-size:1em;text-align:center; color: white; font-family: Circular-black;">
                                        {{now_playing.title}}<span style="opacity:.4;float:right;padding-right:1em;"> Ending {{timeleft(now_playing.end)}}</span>
                                        <span style="opacity:1;float:left;color:red;margin-right:1em;"><span
                                                class="pulse"></span> Now </span>
                                    </h1>
                                    <hr style="opacity:.2;">
                                    <div v-for="(schedule, index) in formated_schedules"
                                         v-if="(between(schedule.start, schedule.end)=='Later')">
                                        <h1 style="font-size:1em;text-align:center;font-family: Circular-black;color: white;">
                                            <span style="opacity:.4;float:right;padding-right:1em;">{{schedule.start}} - {{schedule.end}}</span><span
                                                style="opacity:1;float:left;color:rgb(137,136,136);margin-right:1em;"><i
                                                class="ion-ios-time"></i>&nbsp {{between(schedule.start, schedule.end)}} </span>{{schedule.title}}
                                        </h1>
                                        <hr style="opacity:.2;">
                                    </div>
                                </div>
                        ...
                        <li>
                            ...
                            <ul class="dropdown-menu">
                                <li v-for="cat in categories">
                                    <router-link :to="{ name: 'blogcategory', params: { id: cat.id, name:cat.name }}">
                                        {{cat.name}}
                                    </router-link>
                                </li>
                            </ul>
                        ...
                        <li style="margin-right: -1.1em;">
                            ...
                            <ul class="dropdown-menu">
                                <li><a href="javascript:void()"
                                       @click="change_state('Lagos', 'http://icestream.coolwazobiainfo.com:8000/coolfm-lagos')">Coolfm
                                    Lagos</a></li>
                                <li><a href="javascript:void()"
                                       @click="change_state('Kano', 'http://icestream.coolwazobiainfo.com:8000/coolfm-kano')">Coolfm
                                    Kano</a></li>
                                <li><a href="javascript:void()"
                                       @click="change_state('Abuja', 'http://icestream.coolwazobiainfo.com:8000/coolfm-abuja')">Coolfm
                                    Abuja</a></li>
                                <li><a href="javascript:void()"
                                       @click="change_state('Ph', 'http://icestream.coolwazobiainfo.com:8000/coolfm-ph')">Coolfm
                                    PH</a></li>
                            </ul>
                        </li>
                    </ul>
        ...
        <center>
            <!-- homepage ad -->
            <ins class="adsbygoogle" style="display:block" data-ad-client="ca-pub-8585125296175362"
                 data-ad-slot="2016565894" data-ad-format="auto"></ins>
        </center>
        <br>
        <transition>
        ...
        <div class="navbar navbar-default navbar-fixed-bottom"
             style="background-color: rgba(255, 255, 255, .9) !important">
            <div class="container">
