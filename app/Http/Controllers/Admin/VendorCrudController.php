...
use App\Notifications\vendorApplied;
...
class VendorCrudController extends CrudController
{
    ...
    public function register(Request $request){
        $vendor = new Vendor();
        $vendor->name = $request->input('salutation') . ' ' . $request->input('first_name') . ' ' . $request->input('last_name');
        $vendor->email = $request->input('email');
        $vendor->phone_number = $request->input('phone_number');
        $vendor->address = $request->input('address');
        $vendor->save();
        $vendor->notify(new vendorRegistered($vendor));
        (new User)->where('isadmin', 1)->first()->notify(new vendorApplied($vendor));
        return ['type' => 'success', 'message' => 'you\'ve got mail!, we\'ll get back to you shortly'];
    }
}
